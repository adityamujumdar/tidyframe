    ...
    try:
        ...
        for claim in required_claims:
            if claim not in payload:
                logger.warning("jwt_verification_failed", reason=f"missing_claim_{claim}")
                return None
        
        # SECURITY: Validate token type
        token_type = payload.get("type")
        if token_type not in ["access", "refresh"]:
            logger.warning("jwt_verification_failed", reason="invalid_token_type", type=token_type)
            return None
        ...
        logger.warning("jwt_verification_failed", reason="jwt_error", error_type=error_type, error=str(e))
        return None
    except Exception as e:
...
def create_user_tokens(user_id: str, email: str, is_admin: bool = False) -> Dict[str, str]:
    """
