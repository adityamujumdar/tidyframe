    ...
    if payload:
        ...
        if user_id:
            ...
            if user and not user.is_account_locked():
                logger.debug(f"Authenticated user: {user.email}, is_admin: {user.is_admin}")
                return user
            elif user:
    ...
    if current_user:
        ...
        if anonymous_usage:
            if not anonymous_usage.can_parse(rows_to_parse):
                remaining = max(0, settings.ANONYMOUS_LIFETIME_LIMIT - anonymous_usage.parse_count)
