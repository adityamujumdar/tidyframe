    ...
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            for row in reader:
                ...
                elif any(x in name_lower for x in ['llc', 'inc', 'corp', 'ltd', 'company', 'properties']):
                ...
                elif 'life estate' in name_lower or 'l/e' in name_lower or ' le ' in name_lower:
                    patterns['life_estates'].append(name)
                    
                elif any(prefix in name_lower for prefix in ['van ', 'von ', 'de ', 'mc', 'mac ', "o'"]):
...
def evaluate_prompt_coverage(patterns: Dict[str, List[str]]):
    ...
    ambiguous = [n for n in patterns['simple_persons'] if ' ' in n and len(n.split()) == 2]
    if ambiguous:
        challenges.append(f"Ambiguous two-word names (like 'Cole Beulah'): {len(ambiguous)} cases")
    
    # Check for complex joint patterns
    complex_joints = [n for n in patterns['joint_ownership'] if n.count('&') > 1 or n.count('/') > 1]
    if complex_joints:
        challenges.append(f"Complex joint ownership (multiple separators): {len(complex_joints)} cases")
    
    # Check for unusual patterns
    if patterns['complex_patterns']:
        challenges.append(f"Complex/unusual patterns: {len(patterns['complex_patterns'])} cases")
