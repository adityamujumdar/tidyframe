...
def get_site_password_middleware(request: Request):
    ...
    for middleware in request.app.middleware_stack:
        if hasattr(middleware, 'cls') and middleware.cls.__name__ == 'SitePasswordMiddleware':
...
async def authenticate_site_password(
    password_request: SitePasswordRequest,
    request: Request
):
    """
    ...
    if not enabled:
        return JSONResponse(
            content={
                "success": True,
                "message": "Site password protection is disabled"
            }
        )
    ...
    if not site_password:
        logger.error("Site password authentication attempted but no password configured")
        raise HTTPException(
    ...
    logger.info(
        ...
        first_char_match=password_request.password[0] == site_password[0] if password_request.password and site_password else False,
        last_char_match=password_request.password[-1] == site_password[-1] if password_request.password and site_password else False,
