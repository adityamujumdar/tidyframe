...
from app.core.security import (
    verify_password, get_password_hash, create_user_tokens,
...
from app.api.auth.schemas import (
    UserRegister, UserLogin, TokenResponse, RefreshToken, PasswordReset,
    PasswordResetConfirm, EmailVerify, ChangePassword, UserResponse,
...
async def register_user(
    ...
    try:
        ...
        logger.error("checkout_session_creation_failed_on_registration", 
                    user_id=user.id, 
                    error=str(e))
    ...
    logger.info(
        ...
        user_id=user.id, 
        email=user.email,
        legal_compliance=user.is_legally_compliant() if hasattr(user, 'is_legally_compliant') else False
    )
    ...
    if checkout_url:
        ...
        response_dict["message"] = "Registration successful! Please complete payment to access all features."
...
async def resend_verification_email(
    ...
    send_email_verification.delay(current_user.email, current_user.full_name, verification_token)
...
async def google_callback(
    ...
    try:
        ...
        user_info = await oauth_service.exchange_code_for_token(callback_data.code, callback_data.state)
