...
class StripeService:
    ...
    async def report_usage(self, customer_id: str, quantity: int, 
        ...
        try:
            ...
            logger.info(f"Reported {quantity} usage to meter '{self.meter_name}' for customer {customer_id}")
            return True
    ...
    def construct_webhook_event(self, payload: bytes, signature: str, 
        ...
        try:
            ...
            if webhook_type == "meter":
                secret = self.meter_webhook_secret or settings.STRIPE_BILLING_METER_WEBHOOK_SECRET
            else:
    ...
    def verify_webhook_signature(self, payload: bytes, signature: str, 
                                 is_meter_event: bool = False) -> bool:
    ...
    async def _handle_invoice_created(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        invoice = event['data']['object']
        logger.info(f"Invoice created: {invoice['id']} for ${invoice['amount_due']/100:.2f}")
        return {'status': 'processed', 'invoice_id': invoice['id']}
    
    async def _handle_payment_action_required(self, event: Dict[str, Any]) -> Dict[str, Any]:
        """Handle payment requiring additional action"""
        invoice = event['data']['object']
        logger.warning(f"Payment action required for invoice {invoice['id']}")
        return {'status': 'processed', 'invoice_id': invoice['id'], 'action_required': True}
...
class UsageMeteringService:
    ...
    async def track_usage(self, user_id: str, customer_id: str, 
                         quantity: int, is_admin: bool = False) -> bool:
...
class BillingEnforcementService:
    ...
    async def check_access(self, user_id: str, customer_id: str,
