...
class NameCacheManager:
    ...
    def __init__(self, max_memory_cache: int = 5000, ttl_hours: int = 24):
        ...
        self._similarity_index = defaultdict(set)  # normalized_name -> set of cache_keys
    ...
    def _cleanup_similarity_index(self, cache_key: str) -> None:
        ...
        self._exact_duplicates = {
            k: v for k, v in self._exact_duplicates.items() 
    ...
    def _schedule_disk_write(self, cache_key: str, name_text: str, result: CachedResult) -> None:
        """Schedule disk write for batch processing"""
        with self._batch_lock:
            self._batch_updates.append((cache_key, name_text, result))
            
            # Flush if batch is full or enough time has passed
            if (len(self._batch_updates) >= 50 or 
                time.time() - self._last_batch_flush > 30):
                ...
                for cache_key, name_text, result in self._batch_updates:
                    ...
                    conn.execute(
                        """INSERT OR REPLACE INTO name_cache 
                           ...
                           VALUES (?, ?, ?, ?, ?, ?, ?)""",
                        (cache_key, name_text, normalized_name, result_json,
                         result.cached_at, result.last_accessed, result.access_count)
                    )
    ...
    def cleanup_expired(self) -> int:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    "DELETE FROM name_cache WHERE created_at < ?",
                    (cutoff_time,)
                )
