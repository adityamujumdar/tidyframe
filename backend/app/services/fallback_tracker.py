...
class FallbackTracker:
    ...
    def create_parsing_result(self, 
        ...
        result = {
            ...
            **{k: v for k, v in result_data.items() 
               if k not in ['first_name', 'last_name', 'entity_type', 'gender', 
                          'gender_confidence', 'is_agricultural']}
        }
        
        # Log fallback usage for monitoring
        if parsing_method == "fallback":
            logger.warning("fallback_parsing_used",
                         original_text=original_text,
                         reason=fallback_reason,
                         confidence=confidence,
                         warnings_count=len(warnings))
        elif confidence < 0.7:
            logger.warning("low_confidence_parsing",
                         original_text=original_text,
                         parsing_method=parsing_method,
                         confidence=confidence,
                         warnings_count=len(warnings))
    ...
    def _generate_automatic_warnings(self, 
                                   parsing_method: str,
                                   success: bool,
                                   confidence: float,
                                   fallback_reason: Optional[str]) -> List[str]:
        ...
        if parsing_method == "fallback":
            if fallback_reason:
                ...
                warnings.append(f"Gemini API unavailable ({reason_msg}) - used fallback parsing")
            else:
    ...
    def track_batch_processing(self, 
                             total_rows: int,
                             gemini_used_count: int,
                             fallback_used_count: int,
                             fallback_reasons: Dict[str, int]) -> Dict[str, Any]:
        """Track batch processing statistics"""
        
        batch_stats = {
            'total_rows': total_rows,
            'gemini_success_rate': (gemini_used_count / total_rows * 100) if total_rows > 0 else 0,
            'fallback_usage_rate': (fallback_used_count / total_rows * 100) if total_rows > 0 else 0,
            'fallback_reasons': fallback_reasons,
            'processing_quality': 'high' if gemini_used_count / total_rows > 0.95 else 
                                 'medium' if gemini_used_count / total_rows > 0.8 else 'low'
        }
        
        # Log batch statistics
        logger.info("batch_processing_stats",
                   total_rows=total_rows,
    ...
    def create_warning_summary(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        quality_factors = [
            ...
            (total - summary['results_with_warnings']) / total * 0.5  # Low warning rate
        ]
