...
class DataExportService:
    ...
    async def export_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        logger.info("User data export completed", user_id=user_id, data_categories=len(user_data))
        return user_data
    ...
    async def _export_account_details(self, user: User) -> Dict[str, Any]:
        ...
        return {
            ...
            "month_reset_date": user.month_reset_date.isoformat() if user.month_reset_date else None,
    ...
    async def _export_legal_consent(self, user: User) -> Dict[str, Any]:
        ...
        return {
            ...
            "consent_timestamps": {
                ...
                "arbitration_acknowledged_at": user.arbitration_acknowledged_at.isoformat() if user.arbitration_acknowledged_at else None,
                "location_confirmed_at": user.location_confirmed_at.isoformat() if user.location_confirmed_at else None
            }
        }
    ...
    async def _export_usage_stats(self, user: User) -> Dict[str, Any]:
        ...
        parse_count_stmt = select(func.count(ParseLog.id)).where(ParseLog.user_id == user.id)
        parse_count = await self.db.execute(parse_count_stmt)
        total_parses = parse_count.scalar() or 0
        
        # Get job statistics
        job_count_stmt = select(func.count(ProcessingJob.id)).where(ProcessingJob.user_id == user.id)
    ...
    async def _export_processing_history(self, user: User) -> List[Dict[str, Any]]:
        ...
        ).order_by(ProcessingJob.created_at.desc()).limit(100)
    ...
    async def delete_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        logger.warning("CCPA data deletion requested", user_id=user_id, email=user.email)
        ...
        try:
            ...
            deletion_report["deleted_data_types"].append(f"Parse Logs ({len(parse_logs)})")
            ...
            deletion_report["deleted_data_types"].append(f"Processing Jobs ({len(jobs)})")
