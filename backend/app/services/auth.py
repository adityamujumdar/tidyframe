...
from app.schemas.auth import UserCreate, ConsentData
...
class AuthService:
    ...
    async def create_user(self, user_create: UserCreate, client_ip: str = None) -> User:
        ...
        if user_create.consent:
            consent_time = datetime.fromisoformat(user_create.consent.consent_timestamp.replace('Z', '+00:00'))
    ...
    def _parse_full_name(self, full_name: Optional[str]) -> tuple[Optional[str], Optional[str]]:
    ...
    async def create_oauth_user(self, email: str, google_id: str, full_name: str = None) -> User:
