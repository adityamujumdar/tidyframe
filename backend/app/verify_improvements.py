...
async def verify():
    ...
    try:
        ...
        if hasattr(result, 'results'):
            ...
            for i, (input_name, exp_first, exp_last, exp_type) in enumerate(CRITICAL_TESTS):
                if i < len(results):
                    r = results[i]
                    ...
                    match = (first == exp_first and last == exp_last and entity == exp_type)
                    ...
                    if match:
                        ...
                        print(f"  Expected: F='{exp_first}' L='{exp_last}' T='{exp_type}'")

            accuracy = success_count / len(CRITICAL_TESTS) * 100
            print(f"\n{'='*60}")
            print(f"Result: {success_count}/{len(CRITICAL_TESTS)} correct ({accuracy:.0f}%)")
