...
try:
    ...
    logger.info("loaded_correct_gemini_service", 
...
from app.utils.job_db import update_job_status, update_job_progress_db
...
class OptimizedFileProcessorService:
    ...
    def extract_name_data_optimized(self, df: pd.DataFrame, name_columns: List[str]) -> Tuple[List[str], List[int]]:
        ...
        for idx, row in df.iterrows():
            ...
            combined_name = combined_name.replace(" Other", "").replace("Other ", "").strip()
            ...
            cleaned_tokens = [token for token in name_tokens if len(token.rstrip('.,')) > 1]
    ...
    async def process_file_optimized(self, job_id: str, file_path: str, 
                                   user_id: str = None, 
                                   parsing_config: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if total_rows > settings.MAX_ROWS_PER_FILE:
                raise ValueError(f"File too large: {total_rows} rows (max: {settings.MAX_ROWS_PER_FILE})")
            ...
            if not name_columns:
                raise ValueError("No name columns found in file. Please specify a column to process.")
            ...
            if self.batch_processor is None:
                ...
                raise RuntimeError("Gemini service is not available. Cannot process names.")
            ...
            for r in batch_result.results:
                try:
                    if hasattr(r, 'to_dict') and callable(getattr(r, 'to_dict')):
                        ...
                        result_dict = {}
                        for key in ['first_name', 'last_name', 'entity_type', 'gender', 
                                   'gender_confidence', 'parsing_confidence', 'parsing_method', 
                                   'fallback_reason', 'warnings', 'original_text']:
                            if key == 'warnings':
                        ...
                        logger.warning("unknown_result_type", result_type=type(r).__name__)
                        result_dicts.append({
                            'first_name': '',
                            'last_name': '',
                            'entity_type': 'unknown',
                    ...
                    logger.error("result_conversion_error", error=str(e), result_type=type(r).__name__)
                    result_dicts.append({
                        'first_name': '',
                        'last_name': '',
                        'entity_type': 'unknown',
            ...
            file_paths = await self._save_optimized_results(
                results_df, job_id, batch_result, start_time, result_dicts, warning_summary
            )
            ...
            processing_results = {
                ...
                'success_rate': (batch_result.successful_parses / total_rows) * 100 if total_rows > 0 else 0,
                ...
                'fallback_stats': {
                    'gemini_used': batch_result.gemini_used if hasattr(batch_result, 'gemini_used') else warning_summary.get('gemini_used', 0),
                    'fallback_used': batch_result.fallback_used if hasattr(batch_result, 'fallback_used') else warning_summary.get('fallback_used', 0),
                    'fallback_reasons': warning_summary.get('fallback_reasons', {}),
                    'fallback_rate': (batch_result.fallback_used / total_rows * 100) if total_rows > 0 and hasattr(batch_result, 'fallback_used') else 0
                },
                'warning_stats': {
                    'results_with_warnings': warning_summary['results_with_warnings'],
                    'total_warnings': warning_summary['total_warnings'],
                    'warning_rate': (warning_summary['results_with_warnings'] / total_rows * 100) if total_rows > 0 else 0
                },
                'quality_score': warning_summary['quality_score'],
                'recommendations': warning_summary['recommendations']
            }
            
            update_job_status(job_id, JobStatus.COMPLETED, processing_results=processing_results)
            
            logger.info("optimized_processing_completed",
                       job_id=job_id,
                       total_rows=total_rows,
    ...
    def _identify_name_columns_with_config(self, df: pd.DataFrame, 
                                         parsing_config: Dict[str, Any] = None) -> List[str]:
        ...
        if config and config.primary_name_column and config.primary_name_column in df.columns:
            return [config.primary_name_column]
        elif config and config.name_columns:
            available_columns = [col for col in config.name_columns if col in df.columns]
        ...
        fallback_columns = ['name', 'names', 'full_name', 'fullname', 'owner', 'person', 'client', 'addressee']
    ...
    def _create_optimized_results_dataframe(self, original_df: pd.DataFrame, 
                                          parsing_results: List[Dict[str, Any]], 
                                          name_columns: List[str],
                                          row_indices: List[int]) -> pd.DataFrame:
        ...
        processed_columns = [
            'entity_type', 'first_name', 'last_name',
            'gender', 'gender_confidence', 'parsing_confidence', 
            'parsing_method', 'fallback_reason', 'gemini_used',
            'has_warnings', 'low_confidence',
            'warnings', 'original_name_text'
        ]
        ...
        for col in processed_columns:
            if col == 'warnings':
                ...
                results_data[col] = [
                    '; '.join(result.get(col, [])) if isinstance(result.get(col), list) 
                    else str(result.get(col, ''))
                    for result in parsing_results
                ]
            elif col == 'original_name_text':
                results_data[col] = [result.get('original_text', '') for result in parsing_results]
            elif col in ['gemini_used', 'has_warnings', 'low_confidence']:
                # Boolean fields
                results_data[col] = [result.get(col, False) for result in parsing_results]
            else:
    ...
    async def _save_optimized_results(self, results_df: pd.DataFrame, job_id: str, 
                                    batch_result: Any, start_time: float, result_dicts: List[Dict[str, Any]], 
                                    warning_summary: Dict[str, Any] = None) -> Dict[str, str]:
        ...
        with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            ...
            metrics_data = {
                ...
                'Value': [
                    ...
                    round((batch_result.successful_parses / len(batch_result.results)) * 100, 2),
            ...
            if warning_summary:
                ...
                fallback_df.to_excel(writer, sheet_name='Fallback Analysis', index=False)
    ...
    def _create_entity_analysis(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Create detailed entity analysis"""
        entity_counts = {}
        gender_counts = {}
        confidence_ranges = {'Low (0-0.5)': 0, 'Medium (0.5-0.8)': 0, 'High (0.8-1.0)': 0}
        ...
        for entity_type, count in entity_counts.items():
            ...
            analysis.append({'Category': f'  {entity_type.title()}', 'Value': f'{count} ({pct:.1f}%)'})
        ...
        if gender_counts:
            ...
            for gender, count in gender_counts.items():
                ...
                analysis.append({'Category': f'  {gender.title()}', 'Value': f'{count} ({pct:.1f}%)'})
        ...
        for range_label, count in confidence_ranges.items():
            pct = (count / len(results)) * 100
            analysis.append({'Category': f'  {range_label}', 'Value': f'{count} ({pct:.1f}%)'})
        
        return analysis
    
    def _create_fallback_analysis(self, warning_summary: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        analysis.append({'Category': f'  Gemini API Success', 'Value': f'{gemini_count} ({gemini_pct:.1f}%)'})
        analysis.append({'Category': f'  Fallback Used', 'Value': f'{fallback_count} ({fallback_pct:.1f}%)'})
        analysis.append({'Category': '', 'Value': ''})
        ...
        if fallback_reasons:
            ...
            for reason, count in fallback_reasons.items():
                ...
                analysis.append({'Category': f'  {friendly_reason}', 'Value': f'{count} ({pct:.1f}%)'})
            analysis.append({'Category': '', 'Value': ''})
        ...
        analysis.append({'Category': f'  Results with Warnings', 'Value': f'{results_with_warnings} ({(results_with_warnings/total)*100:.1f}%)'})
        analysis.append({'Category': f'  Total Warnings Generated', 'Value': f'{total_warnings}'})
        analysis.append({'Category': f'  Low Confidence Results', 'Value': f'{low_confidence} ({(low_confidence/total)*100:.1f}%)'})
        analysis.append({'Category': '', 'Value': ''})
        
        # Overall quality score
        quality_score = warning_summary.get('quality_score', 0) * 100
        analysis.append({'Category': 'Overall Quality Score', 'Value': f'{quality_score:.1f}%'})
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(
                processor_service.process_file_optimized(job_id, file_path, user_id, parsing_config)
            )
            return result
        finally:
            # Cleanup
            try:
                if hasattr(processor_service.batch_processor, 'cleanup'):
                    if asyncio.iscoroutinefunction(processor_service.batch_processor.cleanup):
                        loop.run_until_complete(processor_service.batch_processor.cleanup())
                    else:
