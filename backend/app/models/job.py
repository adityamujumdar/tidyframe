...
class ProcessingJob(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True)  # Nullable for anonymous
    ...
    # Fallback tracking
    gemini_success_count = Column(Integer, default=0, nullable=False)  # Rows parsed with Gemini
    fallback_usage_count = Column(Integer, default=0, nullable=False)  # Rows that used fallback
    fallback_reasons = Column(JSONB, nullable=True)  # Detailed fallback reasons
    
    # Quality metrics
    low_confidence_count = Column(Integer, default=0, nullable=False)  # Results with confidence < 0.7
    warning_count = Column(Integer, default=0, nullable=False)  # Total warnings generated
    ...
    parse_logs = relationship("ParseLog", back_populates="job", cascade="all, delete-orphan")
    ...
    def has_quality_concerns(self) -> bool:
        ...
        return (fallback_rate > 10 or 
                low_confidence_rate > 20 or 
                warning_rate > 30)
