        ...
        try:
            ...
            if status == JobStatus.PROCESSING:
                ...
                if processing_results:
                    ...
                    if 'successful_parses' in processing_results and processing_results['successful_parses'] is not None:
                    ...
                    analytics = {
                        ...
                        'gender_distribution': processing_results.get('gender_distribution', {}),
                        'avg_confidence': processing_results.get('avg_confidence', 0.0),
                        'high_confidence_count': processing_results.get('high_confidence_count', 0),
                        'medium_confidence_count': processing_results.get('medium_confidence_count', 0),
                        'low_confidence_count': processing_results.get('low_confidence_count', 0),
                        'success_rate': processing_results.get('success_rate', 0.0),
                        # Add additional fields that frontend expects
                        'gemini_success_count': processing_results.get('fallback_stats', {}).get('gemini_used', 0),
                        'fallback_usage_count': processing_results.get('fallback_stats', {}).get('fallback_used', 0)
                    }
                    job.error_details = analytics  # Repurposing for analytics storage
                    
                    # Calculate processing time
                    if job.started_at:
                        processing_time = (current_time - job.started_at).total_seconds() * 1000
                    ...
                    if job.user_id:
                        ...
                        if user:
                            ...
                            if processing_results and 'successful_parses' in processing_results and processing_results['successful_parses'] is not None:
                                rows_parsed = processing_results['successful_parses']
                            else:
                                rows_parsed = job.processed_rows
                            if rows_parsed and rows_parsed > 0:
                                user.parses_this_month = (user.parses_this_month or 0) + rows_parsed
                                ...
                                if user.stripe_customer_id and not user.is_admin:
                                    try:
                                        ...
                                        logger.info(f"Immediately reported {rows_parsed} usage to Stripe for customer {user.stripe_customer_id}")
                                        
                                    except Exception as e:
                                        logger.error(f"Failed to report usage to Stripe immediately: {e}")
                                        ...
                                        logger.info(f"Fallback: Queued {rows_parsed} usage for customer {user.stripe_customer_id}")
                                ...
                                logger.info("user_parse_count_updated",
                    ...
                    if fallback_stats:
                        ...
                        job.fallback_usage_count = fallback_stats.get('fallback_used', 0)
                        job.fallback_reasons = fallback_stats.get('fallback_reasons', {})
                    
                    # Update quality metrics
                    warning_stats = processing_results.get('warning_stats', {})
                    if warning_stats:
                        job.warning_count = warning_stats.get('total_warnings', 0)
                        job.low_confidence_count = processing_results.get('low_confidence_count', 0)
                    ...
                    if job.anonymous_ip:
                        ...
                        anon_stmt = select(AnonymousUsage).where(AnonymousUsage.ip_address == job.anonymous_ip)
...
def update_job_progress_db(job_id: str, progress: int) -> bool:
    ...
    with SyncSessionLocal() as db:
        try:
            ...
            db.rollback()
            logger.error("job_progress_update_failed", 
                        job_id=job_id, 
                        progress=progress,
                        error=str(e))
            return False