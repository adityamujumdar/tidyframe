...
class NameValidator:
    ...
    def __init__(self):
        ...
        self.problematic_patterns = [
            ...
            re.compile(r"(select|drop|delete|insert|update|create|alter)\s", re.IGNORECASE),
            # HTML/XML tags
            re.compile(r"<[^>]+>"),
        ...
        self.valid_prefixes = ['mr', 'mrs', 'ms', 'dr', 'prof', 'rev', 'hon', 'sir', 'dame']
        self.valid_suffixes = ['jr', 'sr', 'ii', 'iii', 'iv', 'v', 'md', 'phd', 'esq', 'cpa']
        
        # Common business entity types - enhanced patterns
        self.business_entities = [
            'llc', 'inc', 'corp', 'ltd', 'co', 'company', 'corporation', 
            'partnership', 'lp', 'limited', 'enterprises', 'associates', 
            'group', 'holdings', 'investments', 'l.l.c.', 'l.l.c', 'llp',
            'l.l.p.', 'l.l.p', 'limited liability company', 'incorporated',
            'chtd', 'chartered', 'pllc', 'p.l.l.c.', 'p.l.l.c',
            'professional corporation', 'professional llc', 'pc', 'p.c.',
            'properties', 'development', 'construction', 'consulting',
            'services', 'solutions', 'systems', 'technologies', 'tech',
            'capital', 'ventures', 'fund', 'funds', 'management',
            'real estate', 'realty', 'property management', 'farms',
            'farm llc', 'ranch llc', 'agriculture llc', 'ag llc'
        ]
        
        # Trust/estate indicators - enhanced patterns
        self.trust_indicators = [
            'trust', 'family trust', 'revocable trust', 'irrevocable trust',
            'living trust', 'testamentary trust', 'estate', 'et al',
            'charitable trust', 'remainder trust', 'trust fund', 'bypass trust',
            'generation skipping trust', 'gst', 'qtip trust', 'marital trust',
            'credit shelter trust', 'dynasty trust', 'grat', 'grantor trust',
            'trustee', 'trustees', 'trust agreement', 'trust dated',
            'the trust of', 'trust u/a', 'trust dtd', 'trust dt',
            'trust established', 'trust created', 'trust under will'
        ]
    ...
    def validate_parsing_result(self, result: Dict) -> Dict[str, any]:
        ...
        if entity_type == 'person':
            ...
            if any(entity in full_name for entity in self.business_entities):
                validation_warnings.append('Person type but contains business entity indicators')
            ...
            if first_name or last_name:
                validation_warnings.append('Company entity type but names were extracted')
        ...
        agricultural_terms = ['farm', 'ranch', 'agriculture', 'livestock', 'grain', 'dairy']
        has_ag_terms = any(term in original_text for term in agricultural_terms)
        
        if is_agricultural and not has_ag_terms:
            validation_warnings.append('Agricultural flag set but no agricultural terms found')
        elif not is_agricultural and has_ag_terms:
            corrected_result['is_agricultural'] = True
            validation_warnings.append('Added agricultural flag based on detected terms')
    ...
    def detect_name_structure(self, name_text: str) -> Dict[str, any]:
        ...
        special_chars = set(name_text) - set('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ ')
        if special_chars:
    ...
    def detect_entity_type(self, name_text: str) -> Dict[str, any]:
        ...
        """
        if not name_text:
            return {'entity_type': 'unknown', 'parsing_confidence': 0.0, 'indicators': []}
    ...
    def suggest_improvements(self, result: Dict) -> List[str]:
        ...
        if entity_type == 'person':
            if not result.get('first_name'):
                suggestions.append('First name not identified - check if entity type is correct')
        ...
        if result.get('gender') == 'unknown' and entity_type == 'person':
            suggestions.append('Gender could not be determined - consider using additional context')
        
        # Joint name suggestions
        if 'joint_names' in result:
            suggestions.append('Joint name detected - consider processing individual names separately')
