...
def analyze_trust_patterns(file_path):
    ...
    patterns = {"lastname_firstname": [], "firstname_lastname": [], "ambiguous": []}
    ...
    with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
        ...
        for row in reader:
            ...
            if first_is_surname and second_is_firstname and not first_is_firstname:
                patterns["lastname_firstname"].append(
                    f"{words[0]} {words[1]} - {name[:50]}"
                )
            elif first_is_firstname and second_is_surname and not second_is_firstname:
                patterns["firstname_lastname"].append(
                    f"{words[0]} {words[1]} - {name[:50]}"
                )
            else:
                ...
                if second_word.endswith(("a", "y", "ie", "ine", "elle")):
                    patterns["lastname_firstname"].append(
                        f"{words[0]} {words[1]} - {name[:50]} [female ending]"
                    )
...
if total > 0:
    ...
    if len(patterns["lastname_firstname"]) > len(patterns["firstname_lastname"]):
        print(
            "→ Data shows preference for [LastName] [FirstName] pattern in property records"
        )
    else:
